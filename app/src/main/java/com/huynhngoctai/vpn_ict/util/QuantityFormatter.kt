...
package com.huynhngoctai.vpn_ict.util
...
object QuantityFormatter {
    fun formatBytes(bytes: Long): String {
        val context = App.get().applicationContext
        return when {
    ...
    fun formatEpochAgo(epochMillis: Long): String {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N)
            return App.get().applicationContext.getString(R.string.latest_handshake_ago, span.seconds.toString())
        ...
        return App.get().applicationContext.getString(R.string.latest_handshake_ago, joined)
    }
}